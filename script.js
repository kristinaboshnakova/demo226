...
(() => {
  ...
  // OPEN SPECIFIC SLIDE FROM MENU LINKS (href="#stay" + data-stay-go="0..3")
  document.addEventListener("click", (e) => {
    const el = e.target.closest("[data-stay-go]");
    if (!el) return;
  
    const href = el.getAttribute("href");
    // приемаме и пълни URL-и, които завършват на #stay
    ...
    // ✅ ако сме в мобилното меню, първо го затвори (освободи скрола)
    const wasMenuOpen = document.body.classList.contains("menu-open");
    if (wasMenuOpen && typeof closeMenu === "function") closeMenu();
  
    // ✅ изчакай да приключи анимацията/отключването на body скрола
    ...
    setTimeout(() => {
      ...
      // още 1 frame за по-сигурно на iOS
...
// ================================
// FIX: Amenities open from mobile menu
// ================================
document.addEventListener("click", (e) => {
  const el = e.target.closest('a[href^="#amen-"]');
  if (!el) return;
  ...
  // ако сме в мобилното меню – спираме default
  if (wasMenuOpen) e.preventDefault();
